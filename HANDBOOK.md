...
### 📅 2026-08-19 - 任務組日曆指定色與全功能自訂管理 (CRUD) 開發
- **完全清空寫死預設組別**：移除硬編碼之預設任務組別對映，改由使用者完全自主掌控與自訂。
- **任務組 CRUD 全功能管理介面 (`#group-color-modal`)**：於彈窗內新增「➕ 新增自訂任務组」輸入區塊、列表每項支援「✏️ 重命名」、「🗑️ 獨立刪除」與一鍵「🗑️ 清空所有任務組」，支援使用者隨時新增（如：簽證、財務、行程等）並設置指定色彩。
- **三種日曆顯示模式**：支援「組別色彩優先」（標題附帶 🔴/🟡/🟢 死線狀態燈號）、「雙色混合模式」（背景呈現組別色彩，邊框呈現死線警告色）與「死線狀態優先」（維持傳統紅黃綠藍配色）彈性切換。
- **發起新案件自動帶入**：「發起新案件」彈窗會自動讀取並帶入使用者當前自訂的任務組清單（以逗號分隔），大幅提升發起專案之效率與一致性。
