...
> 本系統採用 **Google Apps Script (GAS)** 作為後端核心 API 框架，底層使用 **Google Sheets (試算表)** 進行雲端資料儲存與權限隔離設計。前端架構採用 **Tailwind CSS**，並深度整合 **FullCalendar v6** (日曆模式) 與 **Frappe Gantt** (甘特圖模式)，可直接部署於 **GitHub Pages** 提供無伺服器 (Serverless) 的流暢單頁應用 (SPA) 體驗。此階段全面導入 **樂觀更新 (Optimistic UI)** 機制，達成 0ms 延遲視覺響應；建置了完整的案件與任務編輯、刪除管理功能；新增 **自訂任務組配色與多模式日曆顯示** 及 **甘特圖高對比度深色視圖**。本階段更導入了 **臺灣工作天剩餘計算引擎**（自動扣除國定假日與週末，計算補班日），並針對 **角色權限管理與 GAS API 連線穩定度** 進行了深度優化，確保在多人協作與多 Google 帳號環境下的 UI 渲染與按鈕顯示一致性。
