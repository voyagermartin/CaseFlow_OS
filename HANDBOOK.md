...
- **⚡ 雙向時程偏移計算機 (Drawer Date Calculator)**：在任務詳情抽屜時程設定旁增設「⚡ 時程推算」展開面板。
  - **基準日自動帶入**：預設帶入該專案案件之出發日（`reference_date`），若無出發日則自動預設為今日，亦可手動挑選任意指定日。
  - **雙向推算方向切換**：支援「◀ 往前 (出發前/死線)」與「▶ 往後 (出發後/結案)」方向切換。
  - **偏移天數與工作天過濾**：支援自訂偏移天數，並提供「`[v] 僅計臺灣工作天`」核取切換，自動避開週末與國定假日（包含補班日精準判定）。
  - **即時預覽與一鍵套用**：即時預覽推算之目標日期（附帶星期與推算摘要），並提供「⬅️ 套用為開始日」與「➡️ 套用為截止日」一鍵填入並觸發自動同步。
...
- **📅 2026-09-03 (Phase 9: 抽屜雙向時程偏移計算機、正推/倒算 + 臺灣工作天過濾與一鍵套用)**：
  - **⚡ 雙向時程偏移計算機 (`calculateOffsetDate`)**：在任務詳情抽屜（Task Drawer）時程區塊中加入可折疊式推算小工具面板。支援以專案出發日（`reference_date`）或自訂基準日為起點，進行「行前倒算（◀ 往前）」與「團後順推（▶ 往後）」雙向日期計算。
  - **臺灣工作天與國定假日/補班引擎精確整合**：推算時可自由勾選「`[v] 僅計臺灣工作天`」，自動避開六日、國定假日並計入合法補班日；未勾選時則以日曆天快速累加/遞減。
  ...
  - **全自動代部署更新至 `@48`**：完成代碼編寫與測試後，自動執行 `clasp push`、`clasp deploy -i ...`（版本 `@48`）、`git commit` 與 `git push`，確保雲端 Web App 與本機儲存庫同步至最新版本。
