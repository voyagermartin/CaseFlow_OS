...
- **📅 2026-08-31 (案件封存與解除封存機制、防禦性預設組別、日期防誤判優化與 Mock API 補齊)**：
  ...
  - **無組別防禦回退**：修正當新增或編輯案件不填寫任務組別時，會因無組別導致前端樹狀無法渲染、無法新增待辦之 Bug，實作自動 fallback 至「一般待辦」的防禦機制。
  - **今日截止日期防誤判**：修正截止日 ISO 解析在台北時間過中午即被判定為已逾期之 Bug，將當日截止日時間上限調整至 23:59:59，使其在截止當天維持「🟡 急件」而不會顯示「🔴 已逾期」。
  - **快速新增待辦起始日校正**：將快速新增待辦時預設起始日「自動往前推 10 天」之干擾行為，修正為與截止日同步預設為同一天。
...
- **📅 2026-08-31 (第二部分：待辦勾選極速響應優化與抽屜日期按鈕批次儲存)**：
  ...
  - **👥 編輯案件內建批量人員設定 (`batchUpdateCaseTaskVisibility`)**：已直接整合至「✏️ 編輯案件」彈窗內。開啟編輯案件時即可直接勾選/取消人員（支援全選/取消全選），並提供「➕ 批量加入 (預設)」、「🔄 覆蓋重設」與「⏸️ 不變更現有待辦」同步選項，儲存案件時一鍵將權限變更批量套用到該案件下的所有待辦事項。
