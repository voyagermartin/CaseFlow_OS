...
> 本系統採用 **Google Apps Script (GAS)** 作為後端核心 API 框架，底層使用 **Google Sheets (試算表)** 進行雲端資料儲存與權限隔離設計。前端架構採用 **Tailwind CSS**，並深度整合 **FullCalendar v6** (日曆模式) 與 **Frappe Gantt** (甘特圖模式)，可直接部署於 **GitHub Pages** 提供無伺服器 (Serverless) 的流暢單頁應用 (SPA) 體驗。此階段全面導入 **樂觀更新 (Optimistic UI)** 機制，達成 0ms 延遲視覺響應；建置了完整的案件與任務編輯、刪除管理功能；並新增 **自訂任務組配色與多模式日曆顯示** 及 **甘特圖高對比度深色視圖**，達成極致可視化與個人化管理體驗。
...
### 📅 2026-08-20 - 案件樹狀「剩餘工作天」顯示與臺灣節假日排除功能
- **國定假日與週末排除邏輯**：於前端實作工作日計算引擎，精準對接行政院人事行政總處辦公日曆表，將週休二日及臺灣國定假日（支援 2025、2026、2027 年，如春節、中秋連假等）進行排除，並納入週六補班日（如 2025-02-08）之工作天補回計算。
- **動態工作天提示**：在案件樹狀中，針對有設定期限且未完成之任務，標題後方即時渲染 `(剩 X 工作天)`、`(今日到期)`，或已逾期時顯示為 `(逾期 Y 工作天)`。
- **高對比度緊迫度分色**：依據剩餘工作天數套用對應的 Tailwind 樣式，剩餘大於 2 天顯示灰色（`text-slate-400`），小於等於 2 天及今日到期顯示黃色（`text-yellow-500`），逾期則顯示紅色（`text-red-500`），提供直覺的視覺引導。
